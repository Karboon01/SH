...
class Users extends Controller
{
    ...
    public function deleteRequest(Request $request)
    {
        ...
        DB::table('change_log')->insert([
            'name' => '<tr class="flex gap-5"><td>NAME</td><td>' . $request->name . '</td></tr>
            <tr class="flex gap-5"><td>E-MAIL</td><td>' . $request->email . '</td></tr>
            <tr class="flex gap-5"><td>ADMIN</td><td>' . $request->is_admin . '</td></tr>',
            'table' => 'REQUEST - DELETE',
            'datetime' => now(),
        ]);

        $request = DB::table('requests')->where('id', $request->id)->delete();
