@extends('layouts.app')
@section('content')
    <div class="min-h-screen py-[200px] overflow-x-auto bg-neutral-700">
        ...
        <table class="w-[90%] mx-auto text-base whitespace-nowrap flex flex-col">
            <h2 class="my-[50px] text-5xl">PERSON</h2>
            ...
            <tbody>
                @foreach ($person as $item)
                    <tr class="hover:bg-neutral-600 hover:scale-105 duration-100 flex justify-between items-center text-sm">
                        <td class="px-3 py-2 text-left w-[115px] text-ellipsis">
                            {{ $item->rap_name }}
                        </td>
                        <td class="px-3 py-2 text-center w-[75px] text-ellipsis">
                            {{ $item->name }}
                        </td>
                        <td class="px-3 py-2 text-center w-[110px] text-ellipsis">
                            {{ $item->surname }}
                        </td>
                        <td class="px-3 py-2 text-center w-[155px] text-ellipsis">
                            {{ $item->date_of_birth }}
                        </td>
                        <td class="px-3 py-2  text-center w-[90px] text-ellipsis">
                            {{ $item->email }}
                        </td>
                        <td class="px-3 py-2  text-center w-[90px] text-ellipsis">
                            {{ $item->phone }}
                        </td>
                        <td class="px-3 py-2  text-center w-[210px] text-ellipsis">
                            {{ $item->date }}
                        </td>
                        <td class="px-3 py-2  text-center w-[50px] text-ellipsis">
                            {{ $item->zip }}
                        </td>
                        <td class="px-3 py-2  text-center text-wrap w-[110px] text-ellipsis">
                            {{ $item->address }}
                        </td>
                        <td class="px-3 py-2  text-center text-wrap w-[65px] text-ellipsis">
                            {{ $item->city }}
                        </td>
