@extends('layouts.app')
@section('content')
    <div class="min-h-screen pt-[200px] overflow-x-auto flex items-center justify-center bg-neutral-700">
        <form action="/admin_events_person_edit/{{ $person->id }}" method="post">
            @method('patch')
            @csrf
            <table>
                <tr class="mb-3 flex justify-between gap-5">
                    <td>
                        <label for="rap_name" class="form-label">RAP NAME</label>
                    </td>
                    <td>
